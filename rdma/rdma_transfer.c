...
static int init_endpoint(rdma_endpoint_t *endpoint, struct rdma_cm_id *id,
    ...
    endpoint->id = id;
    aligned_size = align_up_size(data_buffer_size, 4096);
    endpoint->data_buffer_size = aligned_size;
    ...
    endpoint->data_buffer = aligned_alloc(4096, aligned_size);
    if (endpoint->data_buffer == NULL) {
...
static void *server_thread_main(void *arg) {
    ...
    receive_buffer_size = server_args->chunk_size;
    if (event->param.conn.private_data != NULL &&
        event->param.conn.private_data_len == sizeof(uint64_t)) {
        receive_buffer_size = (size_t)be64toh(
            *(const uint64_t *)event->param.conn.private_data);
    }
    if (receive_buffer_size == 0) {
        if (server_args->verbose) {
            fprintf(stderr, "Invalid RDMA receive buffer size: %zu\n",
                    receive_buffer_size);
        }
        rdma_ack_cm_event(event);
        rdma_destroy_id(listen_id);
    ...
    if (init_endpoint(&endpoint, endpoint.id, receive_buffer_size,
...
int run_rdma_file_write_client(const char *server_ip, uint16_t port,
    ...
    if (fstat(file_fd, &file_stat) != 0) {
        close(file_fd);
        cleanup_endpoint(&endpoint);
        return report_error(verbose, "fstat(test file)");
    }
    ...
    conn_param.rnr_retry_count = 7;
    connect_private_data = htobe64((uint64_t)file_stat.st_size);
    conn_param.private_data = &connect_private_data;
    conn_param.private_data_len = sizeof(connect_private_data);
    ...
    remote_length = endpoint.recv_message->length;

    if ((uint64_t)file_stat.st_size > (uint64_t)remote_length) {
        if (verbose) {
            fprintf(stderr,
                    "Remote RDMA buffer too small: file=%zu bytes buffer=%u bytes\n",
                    (size_t)file_stat.st_size, remote_length);
        }
        close(file_fd);
        cleanup_endpoint(&endpoint);
        return -1;
    ...
    while ((bytes_read = read(file_fd, endpoint.data_buffer, chunk_size)) > 0) {
        if (remote_offset + (size_t)bytes_read > (size_t)remote_length) {
            if (verbose) {
                fprintf(stderr,
                        "RDMA write exceeds remote buffer: offset=%zu bytes=%zu buffer=%u\n",
                        remote_offset, (size_t)bytes_read, remote_length);
            }
            close(file_fd);
            cleanup_endpoint(&endpoint);
            return -1;
        ...
        remote_offset += (size_t)bytes_read;
...
int run_rdma_file_write_benchmark(const char *server_ip, uint16_t port,
    ...
    if (stat(filename, &file_stat) != 0) {
        return report_error(verbose, "stat(test file)");
    }
    ...
    server_args.port = port;
    server_args.chunk_size = chunk_size;
    server_args.total_bytes = (size_t)file_stat.st_size;
    server_args.verbose = verbose;
    server_args.status = -1;
